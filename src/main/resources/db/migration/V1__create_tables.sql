...
CREATE TABLE AUTH_PERFIL (
    ...
    CONSTRAINT FK_PERFIL_APLICATIVO FOREIGN KEY (APLICATIVO_ID) REFERENCES APLICATIVOS(id)
);
...
-- 5. TABELA ASSOCIATIVA: AUTH_PERFIL_PERMISSAO (ManyToMany Perfil <-> Permissão)
CREATE TABLE AUTH_PERFIL_PERMISSAO (
    AUTH_PERFIL_ID BIGINT NOT NULL,
    AUTH_PERMISSAO_ID BIGINT NOT NULL,
    CONSTRAINT PK_PERFIL_PERMISSAO PRIMARY KEY (AUTH_PERFIL_ID, AUTH_PERMISSAO_ID),
    CONSTRAINT FK_PER_PER_PERFIL FOREIGN KEY (AUTH_PERFIL_ID) REFERENCES AUTH_PERFIL(id),
    CONSTRAINT FK_PER_PER_PERMISSAO FOREIGN KEY (AUTH_PERMISSAO_ID) REFERENCES AUTH_PERMISSAO(id)
);
